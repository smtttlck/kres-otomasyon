...
if ($islem == 1) {
    ...
    try {
        require_once("../../db.php");
        $conn->exec('DELETE FROM `stok` WHERE `stok`.`id`= ' . $id);
        mesaj('silindi');
...
function yazdir($conn)                   ///////////
{
    ...
    $statement = $conn->query("select * from stok");

    echo ('<div> <h2>Stokları Düzenle</h2>
   <button type="button" class="btn btn-primary float-right" data-toggle="modal" data-target="#insertstok">Stok Ekle</button></div> ');
...
');
    ...
    <td class="adi" >' . $row['adi'] . '</td><td class="kategori" >' . $row['kategori'] . '</td><td class="adet" >' . $row['adet'] . '</td>
    <td><button  type="button" id="stokupdate"  data-toggle="modal" data-target="#updatestok" value="' . $row['id'] . '" class="btn btn-warning update">Güncelle</button>
     <button type="button" id="deletestok" value="' . $row['id'] . '" class="btn btn-danger delete">Sil</buttonbutton> </td>
    </tr>
    ');
    }
