...
if ($islem == 1) {
    ...
    try {
        require_once("../../db.php");
        $conn->exec('DELETE FROM `siniflar` WHERE `siniflar`.`id`= ' . $id);
        mesaj('silindi');
...
function yazdir($conn)                   ///////////
{
    ...
    $statement = $conn->query("select * from siniflar");

    echo ('<div> <h2>Sınıfları Düzenle</h2>
   <button type="button" class="btn btn-primary float-right" data-toggle="modal" data-target="#insertsinif">Sınıf Ekle</button></div> ');
...
');
    ...
    <td class="sinifadi" >' . $row['sinifadi'] . '</td><td class="sinifid" >' . $row['sinifid'] . '</td><td class="ogretmenid" >' . $row['ogretmenid'] . '</td><td>
    <button  type="button" id="sinifupdate"  data-toggle="modal" data-target="#updatesinif" value="' . $row['id'] . '" class="btn btn-warning update">Güncelle</button>
     <button type="button" id="deletesinif" value="' . $row['id'] . '" class="btn btn-danger delete">Sil</buttonbutton> </td>
    </tr>
    ');
    }
