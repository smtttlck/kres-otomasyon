...
function yazdir($conn)                   ///////////
{
    ...
    $statement = $conn->query("select * from kirtasiye");

    echo ('<div> <h2>Kırtasiyeyi Düzenle</h2>
   <button type="button" class="btn btn-primary float-right" data-toggle="modal" data-target="#insertkirtasiye">Veri Ekle</button></div> ');
...
');
    ...
    <td class="ogrencid" >' . $row['ogrencid'] . '</td><td class="stokid" >' . $row['stokid'] . '</td><td class="verilenadet" >' . $row['verilenadet'] . '</td>
    <td><button  type="button" id="kirtasiyeupdate"  data-toggle="modal" data-target="#updatekirtasiye" value="' . $row['id'] . '" class="btn btn-warning update">Güncelle</button>
     <button type="button" id="deletekirtasiye" value="' . $row['id'] . '" class="btn btn-danger delete">Sil</buttonbutton> </td>
    </tr>
    ');
    }
